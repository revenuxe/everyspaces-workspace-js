  ...
  return (
    <section id="contact-form" className="relative py-16 md:py-24 px-4 sm:px-6 lg:px-12 overflow-hidden">
      <div className="absolute inset-0">
        <img src={heroImg.src} alt="" className="w-full h-full object-cover" />
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.15 }} className="relative bg-white/10 backdrop-blur-xl border border-white/15 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-2xl shadow-black/20 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rahul@company.com" required className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <select value={teamSize} onChange={(e) => setTeamSize(e.target.value)} required className={`${inputClass} appearance-none cursor-pointer`}>
                  <option value="" disabled className="text-foreground bg-card">Select team size</option>
            ...
            <div>
              ...
              <div className="relative">
                <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="HSR Layout, Koramangala, Whitefield..." required className={`${inputClass} pr-10`} />
                {location && (
                  <button type="button" onClick={() => setLocation("")} className="absolute right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-primary-foreground/20 flex items-center justify-center hover:bg-primary-foreground/30 transition-colors backdrop-blur-sm">
            ...
            <div>
              ...
              <input type="text" value={business} onChange={(e) => setBusiness(e.target.value)} placeholder="IT Consulting, Marketing Agency..." className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Planned Occupancy Timeline</label>
              <div className="relative">
                <select value={timeline} onChange={(e) => setTimeline(e.target.value)} required className={`${inputClass} appearance-none cursor-pointer`}>
                  <option value="" disabled className="text-foreground bg-card">Select timeline</option>
