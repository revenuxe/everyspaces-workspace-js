...
import { useNavigate, useParams } from "@/compat/react-router-dom";
...
const AdminPropertyForm = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (typesRes.error || amenRes.error || locRes.error) {
        toast({
          title: "Unable to load property form data",
          description: typesRes.error?.message || amenRes.error?.message || locRes.error?.message || "Please refresh and try again.",
          variant: "destructive",
        });
      }
      ...
      if (isEdit) {
        ...
        if (data) {
          ...
          toast({
            ...
            description: "The requested property could not be loaded.",
            variant: "destructive",
          });
    ...
    init();
  }, [propertyId, navigate, isEdit, toast]);
