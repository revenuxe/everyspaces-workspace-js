...
const AdminLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (authError) {
      setError(authError.message || "Unable to sign in. Please check your email and password.");
      setLoading(false);
      return;
    }

    const roleResult = await isCurrentUserAdmin(data.user.id);

    if (roleResult.error) {
      await supabase.auth.signOut();
      setError(`Admin role check failed: ${roleResult.error.message}`);
      setLoading(false);
      return;
    }
