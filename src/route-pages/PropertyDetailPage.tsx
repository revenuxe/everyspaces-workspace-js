...
const PropertyDetailPage = ({ initialProperty = null }: PropertyDetailPageProps) => {
  ...
  const matchedCity = Object.values(cityContent).find(
    (city) => city.city.toLowerCase() === property.city.toLowerCase()
  );
  const matchedArea = allAreas.find(
    (area) =>
      area.city.toLowerCase() === property.city.toLowerCase() &&
      area.name.toLowerCase() === property.area.toLowerCase()
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      <AnimatedSection>
        <InternalLinksSection
          ...
          title={`Explore More Around ${property.area}, ${property.city}`}
          description="Use these internal links to compare this property with its local market, broader city page, and other workspace options."
          links={[
            {
              href: matchedCity ? `/office-space/${matchedCity.citySlug}` : "/areas-we-serve",
              title: `${property.city} Office Space Guide`,
              description: `Understand the wider office market across ${property.city} before narrowing down your final shortlist.`,
            },
            {
              href: matchedArea ? `/office-space/${matchedArea.citySlug}/${matchedArea.slug}` : "/areas-we-serve",
              title: `${property.area} Area Page`,
              description: `See how ${property.area} compares for coworking, managed office, and private workspace demand.`,
            },
            {
              href: "/listings",
              title: "Back to All Listings",
              description: "Compare this space against other available office and coworking options across the platform.",
            },
            {
              href: "/contact",
              title: "Request a Viewing",
              description: "Reach out for a guided shortlist, site visit, or faster commercial office search support.",
            },
          ]}
        />
      </AnimatedSection>
