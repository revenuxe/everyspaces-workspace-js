    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm">No {leadTypeLabels[activeType].toLowerCase()} leads yet</p>
        </div>
      ) : (
        <div className="space-y-3">
...
const CrudTab = ({ table, label, fields }: { table: "property_types" | "amenities" | "locations"; label: string; fields: string[] }) => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    if (error) {
      toast({
        title: `Unable to load ${label.toLowerCase()}s`,
        description: error.message,
        variant: "destructive",
      });
    }
  ...
  const handleSave = async () => {
    const isEmpty = fields.some((field) => !(formData[field] || "").trim());
    if (isEmpty) {
      toast({
        title: "Missing fields",
        description: `Please fill in all ${label.toLowerCase()} fields before saving.`,
        variant: "destructive",
      });
      return;
    }
    ...
    if (editingId) {
      ({ error } = await supabase.from(table).update(formData as any).eq("id", editingId));
    } else {
      ({ error } = await supabase.from(table).insert(formData as any));
    }

    if (error) {
      toast({
        title: `${editingId ? "Update" : "Create"} failed`,
        description: error.message,
        variant: "destructive",
      });
      return;
